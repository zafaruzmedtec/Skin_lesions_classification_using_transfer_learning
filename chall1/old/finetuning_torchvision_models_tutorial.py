...
# 
# In general both transfer learning methods follow the same few runss:
# 
...
# epoch runs a full validation runs. It also keeps track of the best
    ...
    for epoch in range(num_epochs):
        ...
        for phase in ['train', 'val']:
            ...
            for inputs, labels in dataloaders[phase]:
                ...
                if (runs+1) % 100 == 0:
                    print ('runs [{}], Loss: {:.4f}, Acc: {:.2f}' 
                           .format(runs+1, loss.item(), accuracy.item()))
                    ...
                    for tag, value in info.items():
                        logger.scalar_summary(tag, value, runs+1)
            
                    # 2. Log values and gradients of the parameters (histogram summary)
                    for tag, value in model.named_parameters():
                        tag = tag.replace('.', '/')
                        logger.histo_summary(tag, value.data.cpu().numpy(), runs+1)
                        #logger.histo_summary(tag+'/grad', value.grad.data.cpu().numpy(), runs+1)
            
                    # 3. Log training images (image summary)
                    info = { 'images': inputs.view(-1, 28, 28)[:10].cpu().numpy() }
            
                    for tag, images in info.items():
                        logger.image_summary(tag, images, runs+1)

            epoch_loss = running_loss / len(dataloaders[phase].dataset)
            epoch_acc = running_corrects.double() / len(dataloaders[phase].dataset)

            print('{} Loss: {:.4f} Acc: {:.4f}'.format(phase, epoch_loss, epoch_acc))
            print('runs: {:.4f} '.format(runs))
            # deep copy the model
...
# --------------------
# 
# Now that the model structure is correct, the final runs for finetuning
...
######################################################################
# Run Training and Validation runs
# --------------------------------
# 
# Finally, the last runs is to setup the loss for the model, then run the
# training and validation function for the set number of epochs. Notice,
# depending on the number of epochs this runs may take a while on a CPU.
